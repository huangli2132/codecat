...
only interpret files that a completed CLI/REPL/TUI run left under `.codecat/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".codecat" / "runs")
        ...
        session_path = _latest_file(workspace / ".codecat" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".codecat" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
