...
REPO="${CODECAT_REPO:-https://github.com/martin-los/codecat.git}"
INSTALL_DIR="${CODECAT_INSTALL_DIR:-$HOME/.codecat-agent}"
BRANCH="${CODECAT_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[codecat]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[codecat]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[codecat]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[codecat] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        codecat  一键安装                    ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    BIN_DIR="${CODECAT_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/codecat"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/codecat" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "codecat 安装完成！"
    printf "\n"
