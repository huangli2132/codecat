...
from codecat.cli import handle_repl_command
from codecat.features import skills as skillslib
...
def build_agent(tmp_path, outputs):
    ...
    return Pico(
        ...
        workspace=workspace,
        session_store=SessionStore(tmp_path / ".codecat" / "sessions"),
...
def test_prompt_documents_project_skill_frontmatter_contract(tmp_path):
    ...
    prompt = agent.prompt("Create .codecat/skills/audit/SKILL.md")

    assert "When creating Pico skill files" in prompt
    assert ".codecat/skills/<name>/SKILL.md" in prompt
...
def test_project_skill_slash_invocation_runs_inline_session(tmp_path):
    skill_dir = tmp_path / ".codecat" / "skills" / "deploy"
...
---
Use target $ARGUMENTS from ${CODECAT_SKILL_DIR}.
""",
        encoding="utf-8",
    )
...
def test_memory_slash_commands_use_kairos_assets(tmp_path):
    ...
    (tmp_path / ".codecat" / "memory" / "MEMORY.md").write_text(
...
def test_dream_slash_command_consolidates_daily_log_into_memory_files(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            '<tool>{"name":"write_file","args":{"path":".codecat/memory/topics/user-preferences.md","content":"# User Preferences\\n\\n## Notes\\n- Prefers concise reports.\\n"}}</tool>',
    ...
    assert "User preferences" in (tmp_path / ".codecat" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
    assert "Prefers concise reports" in (tmp_path / ".codecat" / "memory" / "topics" / "user-preferences.md").read_text(encoding="utf-8")
...
def test_skill_frontmatter_metadata_and_argument_substitution(tmp_path):
    skill_dir = tmp_path / ".codecat" / "skills" / "audit"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: audit
...
def test_fork_skill_runs_in_isolated_session_and_records_completion(tmp_path):
    skill_dir = tmp_path / ".codecat" / "skills" / "inspect"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: inspect
...
def test_skill_allowed_tools_restricts_inline_execution(tmp_path):
    skill_dir = tmp_path / ".codecat" / "skills" / "readonly"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: readonly
...
def test_disable_model_invocation_skill_returns_expanded_prompt(tmp_path):
    skill_dir = tmp_path / ".codecat" / "skills" / "template"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: template
...
def test_cli_lists_skills_without_calling_model(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "codecat", "--cwd", str(tmp_path)],
        input="/skills\n/exit\n",
        text=True,
        capture_output=True,
        env=env,
