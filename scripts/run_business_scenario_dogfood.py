...
from codecat.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient  # noqa: E402
...
def _build_client_factory(*, config_path=None, provider=None, model=None, base_url=None, api_key=None):
    ...
    if not config.api_key:
        raise ValueError(f"provider {config.name!r} has no api key; configure .codecat.toml or pass --api-key")
...
def build_arg_parser():
    ...
    parser.add_argument("--output-dir", default="/tmp/codecat-business-scenario-dogfood", help="Directory for workspaces and summary artifacts.")
