...
from codecat.testing import ScriptedModelClient  # noqa: E402
from codecat import Pico, SessionStore, WorkspaceContext  # noqa: E402
from codecat.config import resolve_provider_config  # noqa: E402
from codecat.features.skills_runtime import invoke_skill  # noqa: E402
from codecat.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient, ProviderError  # noqa: E402

SUMMARY_JSON = "gate8-real-session-acceptance.json"
SUMMARY_MARKDOWN = "gate8-real-session-acceptance.md"
LIVE_ENV_FLAG = "CODECAT_ACCEPTANCE_LIVE"
...
def _scenario_plan_todo_explore(output_dir, workspace):
    ...
    agent = _build_agent(
        ...
        [
            ...
            '<tool name="write_file" path=".codecat/plans/gate8-plan.md"><content># Gate8 Plan\n- Evidence harness\n</content></tool>',
    ...
    return _finalize(
        ...
        checks=[
            ...
            _check("plan_file", (workspace / ".codecat" / "plans" / "gate8-plan.md").is_file()),
...
def _scenario_skill_inline(output_dir, workspace):
    ...
    skill_dir = workspace / ".codecat" / "skills" / "evidence"
...
def _scenario_resume_continuation(output_dir, workspace):
    ...
    store = SessionStore(workspace / ".codecat" / "sessions")
    first = Pico(
...
def _scenario_security_rejection(output_dir, workspace):
    ...
    old_secret = os.environ.get("CODECAT_ACCEPTANCE_SECRET")
    os.environ["CODECAT_ACCEPTANCE_SECRET"] = "codecat-secret-value-123"
    agent = _build_agent(
        workspace,
        [
            ...
            '<tool>{"name":"run_shell","args":{"command":"echo $CODECAT_ACCEPTANCE_SECRET","timeout":5}}</tool>',
    ...
    try:
        ...
        return _finalize(
            ...
            checks=[
                ...
                _check("secret_redacted", "codecat-secret-value-123" not in trace_text),
            ],
        )
    finally:
        if old_secret is None:
            os.environ.pop("CODECAT_ACCEPTANCE_SECRET", None)
        else:
            os.environ["CODECAT_ACCEPTANCE_SECRET"] = old_secret
...
def _scenario_provider_error_recovery(output_dir, workspace):
    ...
    agent = Pico(
        ...
        workspace=_scenario_workspace(workspace),
        session_store=SessionStore(workspace / ".codecat" / "sessions"),
        approval_policy="auto",
        max_steps=2,
    )
...
def _scenario_live_provider_smoke(output_dir, workspace, include_live=None):
    ...
    agent = Pico(
        ...
        max_steps=1,
        max_new_tokens=64,
        secret_env_names=["CODECAT_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY"],
    )
