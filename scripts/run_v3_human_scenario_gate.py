...
from codecat.evaluation.run_evidence import RunEvidence


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = ROOT / ".codecat.toml"
...
class HumanScenarioRunner:
    def __init__(self, args: argparse.Namespace):
        ...
        self.output_dir = Path(args.output_dir or Path("/tmp") / "codecat-v3-human-scenarios" / stamp).resolve()
    ...
    def r03_release_readiness_skill(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "release"
...
""",
    ...
    def s06_tty_default_tui(self) -> ScenarioResult:
        ...
        checks = [
            ...
            check("mentions_pico_or_tui", "codecat" in (stdout + stderr).lower() or "Textual" in stdout + stderr),
        ]
    ...
    def s10_slash_suggestion_registry(self) -> ScenarioResult:
        ...
        command = self.run_python(
            "S10",
            workspace,
            "from codecat.commands.slash import suggest_commands\n"
    ...
    def s13_model_runtime_switch(self) -> ScenarioResult:
        ...
        checks = [
            ...
            check("no_workspace_config_written", not (workspace / ".codecat.toml").exists()),
        ]
    ...
    def s15_plan_mode_active_artifact(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "如果被拒绝，再写 active plan artifact .codecat/plans/auth-refactor-plan.md，内容为 # Plan。最后 final。"
        )
        repl = f"/plan auth-refactor\n{prompt}\n/exit\n"
        command = self.run_pico("S15", workspace, repl_input=repl, max_steps=5, max_new_tokens=1536, timeout=420)
        commands = [command]
        plan_path = workspace / ".codecat" / "plans" / "auth-refactor-plan.md"
        if not plan_path.is_file():
            continue_prompt = (
                "继续上一个 plan mode。不要再写 src/auth.py。"
                "只调用一次 write_file，path 必须是 .codecat/plans/auth-refactor-plan.md，"
    ...
    def s16_plan_final_gate(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "再 write_file .codecat/plans/cache-plan.md，内容为 # Cache Plan，然后 final。"
        )
        command = self.run_pico("S16", workspace, repl_input=f"/plan cache\n{prompt}\n/exit\n", max_steps=5, max_new_tokens=1024, timeout=360)
        stdout = self.read_log(command.stdout_path)
        plan_path = workspace / ".codecat" / "plans" / "cache-plan.md"
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s17_absolute_plan_path(self) -> ScenarioResult:
        ...
        absolute_plan = workspace / ".codecat" / "plans" / "student-plan.md"
        prompt = "write_file .codecat/plans/student-plan.md，内容为 # Student Plan，然后 final。"
        command = self.run_pico(
            "S17",
            workspace,
        ...
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("absolute_path_normalized", "plan path: .codecat/plans/student-plan.md" in stdout),
    ...
    def s18_plan_path_escape_rejected(self) -> ScenarioResult:
        ...
        command = self.run_pico("S18", workspace, repl_input="/plan student .codecat/plans/../escape.md\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("escape_rejected", "plan path must stay under .codecat/plans/" in stdout),
            check("outside_not_written", not (workspace / ".codecat" / "escape.md").exists()),
        ]
    ...
    def s21_prior_read_required(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "1) 先尝试 patch_file README.md old_text='world' new_text='codecat'，不要先 read_file。"
            "2) 如果被拒绝，再 read_file README.md start=1 end=1。"
            "3) 再 patch_file README.md old_text='world' new_text='codecat'。"
            "4) final。"
        )
        ...
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("readme_patched_after_read", text == "hello codecat\n", text),
        ]
    ...
    def s30_skills_list_local(self) -> ScenarioResult:
        ...
        checks = [
            ...
            check("no_run_created", not any((workspace / ".codecat" / "runs").glob("run_*")) if (workspace / ".codecat" / "runs").exists() else True),
        ]
    ...
    def s32_project_skill_arguments(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "deploy"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: deploy
...
---
请只返回 <final>deploy checked $ARGUMENTS from ${CODECAT_SKILL_DIR}</final>，不要调用工具。
""",
            encoding="utf-8",
        )
    ...
    def s33_skill_allowed_tools_restricts_write(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "readonly"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: readonly
...
""",
    ...
    def s34_fork_skill_keeps_parent_history(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "inspect"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: inspect
...
""",
    ...
    def s35_prompt_only_skill(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "template"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: template
...
""",
    ...
    def s36_invalid_skill_frontmatter_diagnostic(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".codecat" / "skills" / "bad"
        skill_dir.mkdir(parents=True)
    ...
    def s43_remember_daily_log(self) -> ScenarioResult:
        ...
        logs = list((workspace / ".codecat" / "memory" / "logs").rglob("*.md"))
    ...
    def s44_dream_writes_memory(self) -> ScenarioResult:
        ...
        repl = (
            ...
            "/remember Decision: keep Pico artifacts under .codecat/runs\n"
        ...
        memory_root = workspace / ".codecat" / "memory"
    ...
    def s50_path_traversal_and_redaction(self) -> ScenarioResult:
        ...
        env = dict(os.environ)
        env["CODECAT_HUMAN_SECRET"] = secret
        command = self.run_pico(
            "S50",
            workspace,
            prompt=prompt,
            extra=["--secret-env-name", "CODECAT_HUMAN_SECRET"],
            env=env,
            max_steps=5,
            max_new_tokens=1536,
            timeout=360,
    ...
    def latest_events_path(self, workspace: Path) -> Path | None:
        events = sorted((workspace / ".codecat" / "sessions").glob("*.events.jsonl"), key=lambda path: path.stat().st_mtime)
    ...
    def latest_run_dir(self, workspace: Path) -> Path | None:
        runs_dir = workspace / ".codecat" / "runs"
    ...
    def latest_session_id(self, workspace: Path) -> str:
        sessions = sorted((workspace / ".codecat" / "sessions").glob("*.json"), key=lambda path: path.stat().st_mtime)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), help="Pico config file. Defaults to this repo's ignored .codecat.toml.")
